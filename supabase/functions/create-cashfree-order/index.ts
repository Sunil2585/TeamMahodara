...
serve(async (req)=>{
  ...
  try {
    ...
    const appId = isProduction ? Deno.env.get('CASHFREE_PRODUCTION_APP_ID') : Deno.env.get('CASHFREE_SANDBOX_APP_ID');
    const secretKey = isProduction ? Deno.env.get('CASHFREE_PRODUCTION_SECRET_KEY') : Deno.env.get('CASHFREE_SANDBOX_SECRET_KEY');
    const cashfreeApiUrl = isProduction ? 'https://api.cashfree.com/pg/orders' : 'https://sandbox.cashfree.com/pg/orders';
    ...
    const returnUrlBase = isProduction ? 'https://team-mahodaraa.vercel.app/login' : 'http://localhost:3000';
    if (!appId || !secretKey) {
